...
| **`clipflow-media`** | `ffmpeg-sys-next` (9.0.2), `wgpu` 30.0, `cpal` | 视频硬解（D3D11VA）、NV12 零拷贝 GPU 纹理上传、WGSL 颜色矩阵转换着色器、基于音频采样的主时钟调度、波形峰值文件 (`.peak`) 生成。 |
...
- **核对项与验收标准**：
  ...
  2. **双监视器呈现**：
     ...
     - 节目监视器实时呈现时间轴多轨叠加画面；
  3. **硬解与音画同步**：D3D11VA 硬解 NV12 纹理上传无撕裂，`cpal` 音频驱动发声，音画时间差控制在 $\le 20\text{ms}$；
